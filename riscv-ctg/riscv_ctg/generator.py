...
class Generator():
        ...
        while any([len(op_conds[x])!=0 for x in op_conds]+[len(op_comb)!=0]):
            ...
            if op_comb:
                ...
                def comb_constraint(*args):
                    env = dict(zip(self.op_vars, args))
                    return eval(cond, globals(), env)
            ...
            def eval_func(cond):
                env = dict(zip(self.op_vars, op_tuple))
                return eval(cond, globals(), env)
    ...
    def valcomb(self, cgf):
        ...
        while inds:
            ...
            if("#nosat" in req_val_comb):
                ...
                def condition(*argv):
                    env = dict(zip(self.val_vars, argv))
                    return eval(req_val_comb, globals(), env)
                ...
                def eval_func(cond):
                    env = dict(zip(self.val_vars, val_tuple))
                    return eval(cond, globals(), env)
    ...
    def correct_val(self,instr_dict):
        ...
        if self.operation:
            for i in range(len(instr_dict)):
                env = {var: toint(instr_dict[i][var]) for var in self.val_vars}
                correctval = eval(self.operation, globals(), env)
